...
export class GuideComponent extends ComponentBase implements OnInit {
  ...
  ngOnInit() {
    ...
    this.user$ = this.firebaseService.user$.subscribe(
      user => this.handleUserUpdate(user),
      error => this.handleServiceErrors(error),
      () => this.finishRequest(`Finished handling user update.`)
    );
  }
  ...
  downloadBook() {
    ...
    const ebookUrl = 'https://drive.google.com/file/d/19J8eWKc1ff7CA2KgGTHxMOFYe9UVsmg2/view?usp=sharing';
    this.loggingService.log(this.componentName, Severity.Information, `Preparing to open new window: ${ebookUrl}.`);
    window.open(ebookUrl, '_blank');
  }

  createDownloadAnalyticEvent() {
    this.loggingService.log(this.componentName, Severity.Information, `Preparing to create analytic event. Downloading...the guide.`);
