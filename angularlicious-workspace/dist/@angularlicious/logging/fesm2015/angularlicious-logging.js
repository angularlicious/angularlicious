...
class AngularliciousLoggingService {
    ...
    log(source, severity, message, tags) {
        ...
        this.message = message;
        this.timestamp = new Date(Date.now());
        const /** @type {?} */ logEntry = new LogEntry(this.applicationName, this.source, this.severity, this.message, tags);
...
class LogWriter {
     ...
     */
    validateEntry() {
        const /** @type {?} */ validationContext = new ValidationContext();
    ...
    setup() {
        if (this.hasWriter) {
            ...
            if (this.targetEntry.tags && this.targetEntry.tags.length > 0) {
                const /** @type {?} */ tags = this.targetEntry.tags.join(',');
